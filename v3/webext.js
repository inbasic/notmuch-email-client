...
const EventEmitter = function(process = {}, obj) {
  Object.assign(this, {
    callbacks: {},
    ifs: {},
    ignores: {},
  ...
  return {
    ...
    build: () => new Promise((resolve, reject) => {
      ...
      channel.onMessage.addListener(r => {
        ...
        else if (r && (stderr === '' && r.stdout && r.stdout.indexOf && r.stdout.indexOf('[fake-resolve]') !== -1)) {
...
// webext/core/chrome.action.js
{
  const callback = tab => {
    webext.action.emit('clicked', tab);
  };

  webext.action = new EventEmitter({
    clicked: {
      first: () => chrome.action.onClicked.addListener(callback),
      last: () => chrome.action.onClicked.removeListener(callback)
    }
  }, chrome.action);
